// gets canvas element with id 'bridge-chart' then gets context for drawing
let canvas = document.querySelector('#bridge-chart')
let ctx = canvas.getContext('2d')


// defines all data for each bridge
let bridges = [
    {
...
// set of colors that the chart will cycle throug
let chartColors = ['tomato', 'orange', 'dodgerblue', 'mediumseagreen', 'slateblue', 'violet'];

// creates a chart
let chart = new Chart(ctx, {
    type: 'bar', // specifies what type of chart it is
    data: {
        // sets labels as the names of the bridges
        labels: bridges.map(function(bridge) { return bridge.name; }),
        datasets: [{ 
            // label for the dataset (what will be displayed in the legend)
            label: 'Length of Bridge Span in meters',
            // will take the span values from the bridges and set it as the data for the chart
            data: bridges.map(function(bridge) { return bridge.span; }),
            // makes the bars different colors
            backgroundColor: bridges.map(function(_, index) {return chartColors[index % chartColors.length];
            })
        }]
    },
    options: {}
